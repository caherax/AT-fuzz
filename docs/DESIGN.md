...
5. **崩溃检测与去重**
   - 检测程序崩溃（返回码 < 0 或 >= 128）
   - 采用 virgin_crash bitmap 路径对比去重；无覆盖率时 fallback 为 hash 去重
   - 保存崩溃输入和元数据
...
AT-Fuzz 采用模块化设计，将模糊测试流程分解为多个核心组件。这种设计使得每个组件职责单一，便于理解、测试和扩展。
...
**核心组件**：
- **组件1**：测试执行器 (TestExecutor) - [`src/components/executor.py`](../src/components/executor.py)
- **组件2**：执行监控器 (ExecutionMonitor) - [`src/components/monitor.py`](../src/components/monitor.py)
- **组件3**：变异器 (Mutator) - [`src/components/mutator.py`](../src/components/mutator.py)
- **组件4&5**：种子调度器 (SeedScheduler) - [`src/components/scheduler.py`](../src/components/scheduler.py)
- **组件6**：评估器 (Evaluator) - [`src/components/evaluator.py`](../src/components/evaluator.py)

**辅助模块**：
- **检查点管理**：[`src/checkpoint.py`](../src/checkpoint.py) - 用于保存和恢复模糊测试状态
- **工具函数**：[`src/utils.py`](../src/utils.py) - 包含共享内存操作、覆盖率计算等工具函数
- **配置管理**：[`src/config.py`](../src/config.py) - 全局配置管理
- **日志系统**：[`src/logger.py`](../src/logger.py) - 统一日志输出
...
- **返回码 < 0**：Python `subprocess` 表示进程被信号杀死（如 SIGSEGV, SIGABRT）
- **返回码 >= 128**：Shell 惯例，通常表示信号终止（例如 SIGSEGV=139, SIGABRT=134）

这样避免了逐一判定各种信号码的复杂性，能覆盖大多数实际的内存崩溃场景。

#### 4.2.2 去重策略

采用两层去重（类似 AFL++ 的思路）：

1. **主策略**：使用 `virgin_crash` bitmap 和 simplify_trace
   - 把覆盖率 bitmap 简化为"执行过的分支"集合
   - 对比 virgin_crash bitmap，找出是否产生了新的执行路径
   - 只有新路径的 crash 才保存并更新 virgin_crash，防止大量路径重复的 crash 堆积

2. **Fallback**：当无覆盖率时，使用 hash 去重
   - 对覆盖率或 stderr 计算 hash
   - 相同 hash 的 crash 不重复保存

这套去重不保证"漏洞完全唯一"，但能显著减少噪声，保持样本库精炼可用。
...
- **`havoc_iterations`**：Havoc 变异迭代次数，控制变异强度（默认 16）
...
5. **检查点机制**：当前实现支持暂停和恢复，但恢复时会跳过初始种子加载
   - 改进方向：支持更灵活的检查点恢复策略
