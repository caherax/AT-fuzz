...
class TestExecutor:
    ...
    更新版：支持 AFL++ SHM 覆盖率获取
    """
    
    def __init__(self, target_path: str, target_args: str, 
                 timeout: int = None, use_coverage: bool = False):
        """
        初始化执行器
    ...
    def execute(self, input_data: bytes) -> Dict:
        ...
        # ASan 配置：使用特殊退出码 (77) 表示崩溃
        if 'ASAN_OPTIONS' in env:
            env['ASAN_OPTIONS'] += ':exitcode=77:log_path=/tmp/asan'
        else:
            env['ASAN_OPTIONS'] = 'exitcode=77:log_path=/tmp/asan:detect_leaks=0'
        ...
        try:
            ...
            return {
                ...
                'crashed': crashed,
                'timeout': False,
                'stderr': result.stderr[:1000] if result.stderr else b'',
                'coverage': coverage
            }
