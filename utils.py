...
def count_coverage_bits(bitmap: Optional[bytes]) -> int:
    """
    计算 bitmap 中设置的位数（边覆盖数）

    Args:
        bitmap: 覆盖率位图（可能为 None）
    ...
    if not bitmap:
        return 0
    return sum(b.bit_count() for b in bitmap)
...
class CoverageTracker:
    ...
    def get_coverage_count(self) -> int:
        """获取当前总覆盖数"""
        return sum(b.bit_count() for b in self.global_bitmap)
